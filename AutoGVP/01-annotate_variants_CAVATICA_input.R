...
master_tab <- master_tab %>%
  mutate(intervar_adjusted_call = coalesce(intervar_adjusted_call, "Not adjusted, clinVar")) %>%
...
results_tab_abridged <- results_tab_abridged %>%
  mutate(final_call = replace(final_call, final_call == "Likely pathogenic", "Likely_pathogenic")) %>%
  distinct()


# write out to file
write.table(results_tab_abridged, output_tab_abr_file,
